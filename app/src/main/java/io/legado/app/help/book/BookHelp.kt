...
object BookHelp {
     ...
     * 清除已删除书的缓存 解压缓存
     */
    suspend fun scanInvalidCacheSize(): Long {
        return clearInvalidCache(false)
    }

    suspend fun clearInvalidCache(): Long {
        return clearInvalidCache(true)
    }

    private suspend fun clearInvalidCache(delete: Boolean): Long {
        return withContext(IO) {
            var clearSize = 0L
            val bookFolderNames = hashSetOf<String>()
            val originNames = hashSetOf<String>()
            appDb.bookDao.all.forEach {
                clearSize += clearComicCache(it, delete)
            ...
            downloadDir.getFile(cacheEpubFolderName)
                .listFiles()?.forEach { epubFile ->
                    if (!originNames.contains(epubFile.name)) {
                        clearSize += epubFile.size()
                        if (delete) {
                            FileUtils.delete(epubFile.absolutePath)
                        }
                    }
                }
            clearSize += File(ArchiveUtils.TEMP_PATH).size()
            if (delete) {
                FileUtils.delete(ArchiveUtils.TEMP_PATH)
            }
            val filesDir = appCtx.filesDir
            clearSize += deleteWithSize(File("$filesDir/shareBookSource.json"), delete)
            clearSize += deleteWithSize(File("$filesDir/shareRssSource.json"), delete)
            clearSize += deleteWithSize(File("$filesDir/books.json"), delete)
            clearSize
        }
    }

    private fun deleteWithSize(file: File, delete: Boolean): Long {
        val size = file.size()
        if (delete) {
    ...
    //清除已经看过的漫画数据
    private fun clearComicCache(book: Book, delete: Boolean): Long {
        //只处理漫画
        //为0的时候，不清除已缓存数据
