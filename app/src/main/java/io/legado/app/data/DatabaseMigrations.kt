...
object DatabaseMigrations {
    ...
    private val migration_93_94 = object : Migration(93, 94) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE books ADD COLUMN pinTime INTEGER NOT NULL DEFAULT 0")
        }
    }
    ...
    private val migration_92_93 = object : Migration(92, 93) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                """
                CREATE TABLE IF NOT EXISTS `book_group_orders` (
                    `groupId` INTEGER NOT NULL,
                    `bookUrl` TEXT NOT NULL,
                    `order` INTEGER NOT NULL,
                    PRIMARY KEY(`groupId`, `bookUrl`)
                )
                """.trimIndent()
            )
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_book_group_orders_bookUrl` ON `book_group_orders` (`bookUrl`)")
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_book_group_orders_groupId` ON `book_group_orders` (`groupId`)")
            db.execSQL(
                """
                INSERT OR IGNORE INTO `book_group_orders`(`groupId`, `bookUrl`, `order`)
                SELECT book_groups.groupId, books.bookUrl, books.`order`
                FROM books
                JOIN book_groups ON book_groups.groupId > 0
                WHERE (books.`group` & book_groups.groupId) > 0
                """.trimIndent()
            )
        }
    }
    ...
    private val migration_90_91 = object : Migration(90, 91) {
        override fun migrate(db: SupportSQLiteDatabase) {
            ...
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_book_tag_refs_tagId` ON `book_tag_refs` (`tagId`)")
        }
    }

    private val migration_91_92 = object : Migration(91, 92) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_book_tag_refs_bookUrl` ON `book_tag_refs` (`bookUrl`)")
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_book_tag_refs_tagId_bookUrl` ON `book_tag_refs` (`tagId`, `bookUrl`)")
        }
    }
