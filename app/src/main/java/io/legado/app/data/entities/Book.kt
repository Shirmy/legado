...
data class Book(
    ...
    fun setSplitLongChapter(limitLongContent: Boolean) {
        ...
        config.splitLongChapterModified = true
    }

    fun getSplitLongChapter(): Boolean {
        return config.splitLongChapterModified && config.splitLongChapter
    }
    ...
    fun deleteAllRecords(deleteOriginal: Boolean = false) {
        val books = if (isLocal && originName.isNotBlank()) {
            (appDb.bookDao.getLocalBooksByFileName(originName) + this).distinctBy { it.bookUrl }
        } else {
            listOf(this)
        }
        books.forEach { book ->
            book.deleteRelatedRecords()
            if (book.isLocal) {
                LocalBook.deleteBook(book, deleteOriginal && book.bookUrl == bookUrl)
            }
        }
        books.forEach {
            it.delete()
    ...
    private fun deleteRelatedRecords() {
        BookHelp.clearCache(this)
        RuleBigDataHelp.deleteBook(bookUrl)
        appDb.bookChapterDao.delByBook(bookUrl)
        appDb.bookTagRefDao.deleteByBook(bookUrl)
        appDb.bookGroupOrderDao.deleteByBook(bookUrl)
        appDb.bookmarkDao.deleteByBook(name, author)
        appDb.readRecordDao.deleteByName(name)
        appDb.searchBookDao.clearByBookUrl(bookUrl)
        coverUrl?.let { FileUtils.delete(it) }
        customCoverUrl?.let { FileUtils.delete(it) }
    }
    ...
    data class ReadConfig(
        ...
        var splitLongChapterModified: Boolean = false,
