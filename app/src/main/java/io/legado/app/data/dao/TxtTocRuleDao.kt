...
interface TxtTocRuleDao {
    ...
    @Query("select * from txtTocRules order by serialNumber, id")
    fun observeAll(): Flow<List<TxtTocRule>>

    @get:Query("select * from txtTocRules order by serialNumber, id")
    val all: List<TxtTocRule>

    @get:Query("select * from txtTocRules where enable = 1 order by serialNumber, id")
    val enabled: List<TxtTocRule>

    @get:Query("select * from txtTocRules where enable = 1 order by serialNumber, id limit 1")
    val defaultEnabled: TxtTocRule?
