...
class TextFile(private var book: Book) {
                    ...
                    if (book.getSplitLongChapter() && curOffset + chapterLength - lastStart > maxLengthWithToc) {
                        ...
                        if (toc.isEmpty()) { //如果当前没有章节，那么就是序章
                            ...
                            if (chapterContent.isNotBlank()) {
                                ...
                                book.intro = if (chapterContentLength > 2000 && title.isNotEmpty()) {
     ...
     * 获取目录规则
     */
    private fun getTocRule(): TxtTocRule? {
        var rule = appDb.txtTocRuleDao.defaultEnabled
        if (rule == null && appDb.txtTocRuleDao.count == 0) {
            DefaultData.importDefaultTocRules()
            rule = appDb.txtTocRuleDao.defaultEnabled
        }
        return rule
    }
