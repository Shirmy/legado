...
class BooksFragment() : BaseFragment(R.layout.fragment_books),
    ...
    override fun onFragmentCreated(view: View, savedInstanceState: Bundle?) {
        arguments?.let {
            ...
            val sort = it.getInt("bookSort", 0)
            bookSort = if (sort == 1 || sort == 3 || sort == 4) 0 else sort
            enableRefresh = false
...
//        })
    ...
    fun upBookSort(sort: Int) {
        binding.root.post {
            val realSort = if (sort == 1 || sort == 3 || sort == 4) 0 else sort
            arguments?.putInt("bookSort", realSort)
            bookSort = realSort
            upRecyclerData()
        }
    }
