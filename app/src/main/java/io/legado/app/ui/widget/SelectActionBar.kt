...
class SelectActionBar @JvmOverloads constructor(
    ...
    fun setRevertActionText(@StringRes id: Int, action: () -> Unit) = binding.run {
        btnRevertSelection.setText(id)
        btnRevertSelection.setOnClickListener { action.invoke() }
        btnRevertSelection.visible()
    }

    fun setEndActionText(@StringRes id: Int, action: () -> Unit) = binding.run {
        btnSelectActionEnd.setText(id)
        btnSelectActionEnd.setOnClickListener { action.invoke() }
        btnSelectActionEnd.visible()
    }

    fun hideRevertAction() = binding.run {
        btnRevertSelection.gone()
    }
    ...
    private fun setMenuClickable(isClickable: Boolean) = binding.run {
        ...
        btnSelectActionEnd.isEnabled = isClickable
        btnSelectActionEnd.isClickable = isClickable
        if (isClickable) {
