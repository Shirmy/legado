...
class ReadBookActivity : BaseReadBookActivity(),
    ...
    private val replaceActivity =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
            if (it.resultCode == RESULT_OK) {
                onReplaceRuleGlobalStateChanged()
            }
        }
    private val searchContentActivity =
        registerForActivityResult(StartActivityContract(SearchContentActivity::class.java)) {
            val data = it.data ?: return@registerForActivityResult
            if (data.getBooleanExtra("showEffectiveReplaces", false)) {
                showEffectiveReplaces()
                return@registerForActivityResult
            }
            val key = data.getLongExtra("key", System.currentTimeMillis())
            val index = data.getIntExtra("index", 0)
            val effectiveReplaceRuleId = data.getLongExtra("effectiveReplaceRuleId", -1L)
                .takeIf { it >= 0L }
            val effectiveReplaceRuleName = data.getStringExtra("effectiveReplaceRuleName")
            val searchResult = IntentData.get<SearchResult>("searchResult$key")
            viewModel.searchResultListKey = data.getStringExtra("searchResultListKey")
            val searchResultList = viewModel.searchResultList?.takeIf {
                    it.firstOrNull()?.query == searchResult?.query &&
                            effectiveReplaceRuleId == viewModel.searchEffectiveReplaceRuleId
                }
                ?: searchResult?.let { listOf(it) }
            if (searchResult != null && searchResultList != null) {
                viewModel.searchContentQuery = searchResult.query
                viewModel.searchEffectiveReplaceRuleId = effectiveReplaceRuleId
                viewModel.searchEffectiveReplaceRuleName = effectiveReplaceRuleName
                viewModel.searchResultList = searchResultList
                val searchResultIndex = searchResultList.indexOf(searchResult).takeIf { it >= 0 }
                    ?: index.coerceIn(0, searchResultList.lastIndex)
                binding.searchMenu.upSearchResultList(searchResultList)
                binding.searchMenu.setEffectiveReplaceRule(
                    effectiveReplaceRuleId,
                    effectiveReplaceRuleName
                )
                isShowingSearchResult = true
                viewModel.searchResultIndex = searchResultIndex
        ...
        for (i in 0 until menu.size) {
            ...
            when (item.groupId) {
                ...
                R.id.menu_group_epub -> item.isVisible = book.isEpub
                    ...
                    R.id.menu_effective_replace_full -> item.isChecked =
                        AppConfig.effectiveReplaceFull
...
//            }
            ...
            R.id.menu_effective_replace_full -> {
                item.isChecked = !item.isChecked
                AppConfig.effectiveReplaceFull = item.isChecked
            }
    ...
    override fun openSearchActivity(searchWord: String?) {
        openSearchActivity(searchWord, null, null)
    }

    override fun openSearchActivity(
        searchWord: String?,
        effectiveReplaceRuleId: Long?,
        effectiveReplaceRuleName: String?
    ) {
        val book = ReadBook.book ?: return
        searchContentActivity.launch {
            ...
            effectiveReplaceRuleId?.let {
                putExtra("effectiveReplaceRuleId", it)
            }
            effectiveReplaceRuleName?.let {
                putExtra("effectiveReplaceRuleName", it)
            }
            viewModel.searchResultList?.first()?.let {
                if (
                    it.query == viewModel.searchContentQuery
                    && effectiveReplaceRuleId == viewModel.searchEffectiveReplaceRuleId
                ) {
                    putExtra("useSearchResultList", true)
                    val resultListKey = viewModel.searchResultListKey
                    if (resultListKey != null) {
                        putExtra("searchResultListKey", resultListKey)
                    } else {
    ...
    override fun exitSearchMenu() {
        if (isShowingSearchResult) {
            ...
            viewModel.searchEffectiveReplaceRuleId = null
            viewModel.searchEffectiveReplaceRuleName = null
        }
    }
    ...
    override fun onMenuShow() {
        ...
        binding.readMenu.setReplacePurifyState(ReadBook.book?.getUseReplaceRule() ?: false)
    }
    ...
    override fun changeReplaceRuleState() {
        ReadBook.book?.let {
            ...
            binding.readMenu.setReplacePurifyState(it.getUseReplaceRule())
            viewModel.replaceRuleChanged()
        }
    }

    override fun enableAllReplaceRules() {
        Coroutine.async {
            val disabledCount = appDb.replaceRuleDao.disabledCount
            if (disabledCount > 0) {
                appDb.replaceRuleDao.enableAll(true)
            }
            disabledCount
        }.onSuccess { disabledCount ->
            if (disabledCount > 0) {
                onReplaceRuleGlobalStateChanged()
                toastOnUi("已启用${disabledCount}条替换规则")
            } else {
                binding.readMenu.upReplaceRuleDisabledState()
                toastOnUi("替换规则已全部启用")
            }
        }
    }

    fun onReplaceRuleGlobalStateChanged() {
        binding.readMenu.upReplaceRuleDisabledState()
        Coroutine.async {
