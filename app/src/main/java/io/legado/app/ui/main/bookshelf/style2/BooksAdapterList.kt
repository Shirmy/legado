...
class BooksAdapterList(context: Context, callBack: CallBack) :
                ...
                for (i in payloads.indices) {
                    ...
                    bundle.keySet().forEach {
                        when (it) {
                            ...
                            "wordCount" -> upWordCount(item)
                            "pinTime" -> ivPin.visible(item.pinTime > 0L)
                            "cover" -> ivCover.load(
                                item,
                                false
                            )
    ...
    private fun ItemBookshelfListBinding.upWordCount(item: Book) {
    ...
    private fun ItemBookshelfList2Binding.upWordCount(item: Book) {
        val wordCount = BookSort.displayWordCount(item)
        if (wordCount.isNullOrBlank()) {
            tvWordCount.gone()
        } else {
