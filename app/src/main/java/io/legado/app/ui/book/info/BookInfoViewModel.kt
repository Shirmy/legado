...
class BookInfoViewModel(application: Application) : BaseViewModel(application) {
    ...
    fun updateCoverByRule(book: Book) {
        execute {
            waitDialogData.postValue(true)
            val coverUrl = BookCover.searchCover(book)
            if (coverUrl.isNullOrBlank()) {
                context.toastOnUi("未搜索到封面")
                return@execute
            }
            var options = RequestOptions().set(OkHttpModelLoader.loadOnlyWifiOption, false)
            if (book.origin.isNotBlank()) {
                options = options.set(OkHttpModelLoader.sourceOriginOption, book.origin)
            }
            val target = ImageLoader.load(context, coverUrl).apply(options).submit()
            try {
                target.get()
            } finally {
            ...
            context.toastOnUi("获取封面失败\n${it.localizedMessage}")
        }.onFinally {
            waitDialogData.postValue(false)
        }
    }

    fun searchBookNameInBrowser(book: Book) {
        val searchQuery = "${book.name} ${book.author}"
        ...
        execute {
            ...
            val novelId = Regex("""(?:https?://)?(?:m\.)?jjwxc\.(?:net|com)/(?:book2|onebook\.php\?novelid=)/?(\d+)""")
                .find(html)?.groupValues?.getOrNull(1)
                ?: Regex("""(?:https?://)?(?:m\.)?jjwxc\.(?:net|com)/(?:book2|onebook\.php\?novelid=)/?(\d+)""")
                    .find(decodedHtml)?.groupValues?.getOrNull(1)
                ?: Regex("""https?://www\.jjwxc\.net/onebook\.php\?novelid=(\d+)""")
                    .find(html)?.groupValues?.getOrNull(1)
            val gongziId = Regex("""(?:https?://)?(?:www\.)?gongzicp\.com/novel-(\d+)\.html""")
                .find(html)?.groupValues?.getOrNull(1)
                ?: Regex("""(?:https?://)?(?:www\.)?gongzicp\.com/novel-(\d+)\.html""")
                    .find(decodedHtml)?.groupValues?.getOrNull(1)
            when {
                novelId != null -> "https://www.jjwxc.net/onebook.php?novelid=$novelId"
                gongziId != null -> "https://www.gongzicp.com/novel-$gongziId.html"
                else -> searchUrl
            }
        }.onSuccess {
            context.openUrl(it)
        }.onError {
            AppLog.put("打开书名浏览器搜索失败\n${it.localizedMessage}", it)
            context.openUrl(searchUrl)
        }
    }
    ...
    fun topBook() {
        execute {
            bookData.value?.let { book ->
                book.pinTime = if (book.pinTime > 0L) 0L else System.currentTimeMillis()
                appDb.bookDao.update(book)
                bookData.postValue(book)
            }
        }
    ...
    fun delBook(deleteOriginal: Boolean = false, success: (() -> Unit)? = null) {
        execute {
            bookData.value?.let {
                it.deleteAllRecords(deleteOriginal)
                inBookshelf = false
            }
        }.onSuccess {
            success?.invoke()
