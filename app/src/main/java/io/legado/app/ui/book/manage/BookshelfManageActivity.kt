...
class BookshelfManageActivity :
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        initGroupData()
        upBookDataByGroupId()
        intent.getStringExtra("searchKey")?.takeIf { it.isNotBlank() }?.let {
    ...
    override fun onPrepareOptionsMenu(menu: Menu): Boolean {
        this.menu = menu
        menu.findItem(R.id.menu_book_group)?.isVisible = false
    ...
    override fun onClickSelectBarMainAction() {
        selectGroup(addToGroupRequestCode, adapter.selectionCommonGroupId())
    }
    ...
    private fun initOtherView() {
        binding.selectActionBar.setRevertActionText(R.string.delete) {
            alertDelSelection()
        }
        binding.selectActionBar.setMainActionText(R.string.add_to_group)
        binding.selectActionBar.setEndActionText(R.string.add_tag) {
            batchAddTags()
        }
        ...
        booksFlowJob = lifecycleScope.launch {
            ...
            appDb.bookDao.flowByGroup(viewModel.groupId).map { list ->
                val sorted = when (bookSort) {
                    ...
                    8 -> {
                        val orderMap = BookGroupOrderHelp.orderMap(viewModel.groupId)
                        if (orderMap.isEmpty()) {
    ...
    private fun upBookData() {
        books?.let { books ->
            ...
            if (searchKey.isNullOrEmpty()) {
                ...
                books.filter {
                    it.name.contains(searchKey.toString()) || it.author.contains(searchKey.toString())
                }.let {
                    adapter.setItems(it)
                }
            }
    ...
    override fun onMenuItemClick(item: MenuItem?): Boolean {
        when (item?.itemId) {
            ...
            R.id.menu_add_to_group -> selectGroup(addToGroupRequestCode, adapter.selectionCommonGroupId())
            R.id.menu_remove_to_group -> selectGroup(removeToGroupRequestCode, 0)
            R.id.menu_add_tags -> batchAddTags()
    ...
    override fun selectGroup(requestCode: Int, groupId: Long) {
        showDialogFragment(
            GroupSelectDialog(groupId, requestCode, singleSelect = true)
        )
    }
        ...
        when (requestCode) {
            ...
            adapter.groupRequestCode -> {
                adapter.actionItem?.let {
                    BookGroupOrderHelp.sync(it.bookUrl, it.group, groupId)
            ...
            removeToGroupRequestCode -> adapter.selection.let { books ->
                BookGroupOrderHelp.syncRemovedFromGroup(groupId, books)
                val array = Array(books.size) { index ->
                    val book = books[index]
    ...
    private fun batchAddTags() {
        val selectedBooks = adapter.selection
        if (selectedBooks.isEmpty()) return
        lifecycleScope.launch {
            val selectedTagIds = selectedBooks
                .flatMap { appDb.bookTagRefDao.getTagIdsByBook(it.bookUrl) }
                .distinct()
                    ...
                    withContext(IO) {
                        selectedBooks.forEach { book ->
                            checkedIds.forEach { tagId ->
                                if (!appDb.bookTagRefDao.hasTag(book.bookUrl, tagId)) {
            ...
            dialog.show(supportFragmentManager, "tagSelect")
        }
    }

    private fun batchRemoveTags() {
        val selectedBooks = adapter.selection
        if (selectedBooks.isEmpty()) return
        lifecycleScope.launch {
            val dialog = io.legado.app.ui.book.tag.TagSelectDialog(
                ...
                lifecycleScope.launch {
                    withContext(IO) {
                        selectedBooks.forEach { book ->
                            checkedIds.forEach { tagId ->
                                appDb.bookTagRefDao.delete(book.bookUrl, tagId)
                            }
                        }
                    }
                }
            }
            dialog.show(supportFragmentManager, "tagRemove")
        }
    }
