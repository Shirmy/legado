...
class SearchMenu @JvmOverloads constructor(
    ...
    private var effectiveReplaceRuleId: Long? = null
    private var effectiveReplaceRuleName: String? = null
    ...
    fun setEffectiveReplaceRule(ruleId: Long?, ruleName: String?) {
        effectiveReplaceRuleId = ruleId
        effectiveReplaceRuleName = ruleName
        val isEffectiveReplaceSearch = ruleId != null
        val title = ruleName?.takeIf { it.isNotBlank() }
            ?: context.getString(R.string.effective_replaces)
        binding.tvMainMenu.setText(
            if (isEffectiveReplaceSearch) context.getString(R.string.effective_replaces)
            else context.getString(R.string.menu)
        )
        binding.llMainMenu.contentDescription =
            if (isEffectiveReplaceSearch) title else context.getString(R.string.menu)
        binding.ivMainMenu.contentDescription = binding.llMainMenu.contentDescription
        updateSearchInfo()
    }
    ...
    fun updateSearchInfo() {
        ReadBook.curTextChapter?.let {
            val ruleInfo = effectiveReplaceRuleName
                ?.takeIf { name -> name.isNotBlank() }
                ?.let { name -> " / 规则: $name" }
                ?: ""
            binding.tvCurrentSearchInfo.text =
                """${context.getString(R.string.search_content_size)}: ${searchResultList.size}$ruleInfo / 当前章节: ${it.title}"""
        }
    }
    ...
    private fun bindEvent() = binding.run {
        ...
        llMainMenu.setOnClickListener {
            runMenuOut {
                if (effectiveReplaceRuleId != null) {
