...
class ReadMenu @JvmOverloads constructor(
    ...
    private fun initView(reset: Boolean = false) = binding.run {
        setReplacePurifyState(ReadBook.book?.getUseReplaceRule() ?: AppConfig.replaceEnableDefault)
        fabReplaceRule.setImageResource(R.drawable.ic_find_replace)
        fabReplaceRule.contentDescription = context.getString(R.string.replace_rule_title)
        fabNightTheme.setImageResource(R.drawable.ic_effective_replace)
        fabNightTheme.contentDescription = context.getString(R.string.effective_replaces)
        initAnimation()
    ...
    private fun bindEvent() = binding.run {
        ...
        fabReplaceRule.setOnLongClickListener {
            callBack.enableAllReplaceRules()
            true
        }
        ...
        fabNightTheme.setOnClickListener {
            runMenuOut {
                callBack.showEffectiveReplaces()
            }
        }
    ...
    fun setReplacePurifyState(enabled: Boolean) = binding.run {
        fabAutoPage.setImageResource(
            if (enabled) R.drawable.ic_cfg_replace else R.drawable.ic_cfg_replace_off
        )
        fabAutoPage.contentDescription = context.getString(R.string.replace_purify)
        setReadFabStateStyle(
            fab = fabAutoPage,
            active = enabled,
            activeIconColor = context.accentColor,
            inactiveIconColor = context.buttonDisabledColor
        )
    }

    fun upReplaceRuleDisabledState() = binding.run {
        Coroutine.async {
            appDb.replaceRuleDao.disabledCount > 0
        }.onSuccess { hasDisabled ->
            setReadFabStateStyle(
                fab = fabReplaceRule,
                active = hasDisabled,
                activeIconColor = context.accentColor,
                inactiveIconColor = textColor
            )
        }
    }

    private fun setReadFabStateStyle(
        fab: FloatingActionButton,
        active: Boolean,
        activeIconColor: Int,
        inactiveIconColor: Int
    ) {
        if (AppConfig.isEInkMode) {
            fab.backgroundTintList = ColorStateList.valueOf(if (active) Color.BLACK else Color.WHITE)
            fab.imageTintList = ColorStateList.valueOf(if (active) Color.WHITE else Color.BLACK)
        } else {
            fab.backgroundTintList = bottomBackgroundList
            fab.imageTintList = ColorStateList.valueOf(
                if (active) activeIconColor else inactiveIconColor
            )
        }
    }
