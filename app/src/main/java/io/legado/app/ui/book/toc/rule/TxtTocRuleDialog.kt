...
class TxtTocRuleDialog() : BaseDialogFragment(R.layout.dialog_toc_regex),
    ...
    private var userSelectedRule = false
    ...
    private fun initSelectedName(tocRules: List<TxtTocRule>) {
        ...
        if (userSelectedRule && selectedRuleId != null && tocRules.any { it.id == selectedRuleId }) {
            return
        }
        durRegex?.let { regex ->
            tocRules.firstOrNull {
                regex == it.rule + TextFile.spaceChars + it.replacement
            }?.let {
        ...
        (tocRules.firstOrNull { it.enable } ?: tocRules.first()).let {
            selectedName = it.name
            selectedRuleId = it.id
        }
    }
    ...
    inner class TocRegexAdapter(context: Context) :
        ...
        val diffItemCallBack = object : DiffUtil.ItemCallback<TxtTocRule>() {
            ...
            override fun areContentsTheSame(oldItem: TxtTocRule, newItem: TxtTocRule): Boolean {
                ...
                if (oldItem.serialNumber != newItem.serialNumber) {
                    return false
                }
                return true
            }
            ...
            override fun getChangePayload(oldItem: TxtTocRule, newItem: TxtTocRule): Any? {
                ...
                if (oldItem.serialNumber != newItem.serialNumber) {
                    payload.putBoolean("upOrder", true)
                }
                if (payload.isEmpty) {
                    return null
                }
        ...
        override fun convert(
            ...
            binding.apply {
                if (payloads.isEmpty()) {
                    ...
                    rbRegexName.isChecked = item.id == selectedRuleId
                    ...
                    for (i in payloads.indices) {
                        ...
                        bundle.keySet().forEach {
                            when (it) {
                                ...
                                "upSelect" -> rbRegexName.isChecked = item.id == selectedRuleId
                            }
                        }
                    }
                }
