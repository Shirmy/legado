...
class GroupEditDialog() : BaseDialogFragment(R.layout.dialog_book_group_edit) {
    ...
    override fun onFragmentCreated(view: View, savedInstanceState: Bundle?) {
        ...
        bookGroup?.let {
            ...
            val spinnerPos = bookSortToSpinnerPos(it.bookSort)
            if (spinnerPos !in 0..<binding.spSort.count) {
                it.bookSort = -1
            }
            binding.spSort.setSelection(bookSortToSpinnerPos(it.bookSort))
            binding.cbEnableRefresh.isChecked = false
            binding.cbEnableOnlyRead.isChecked = false
        } ?: let {
        ...
        binding.run {
            ...
            btnOk.onClick {
                ...
                if (groupName.isNullOrEmpty()) {
                    toastOnUi("鍒嗙粍鍚嶇О涓嶈兘涓虹┖")
                } else {
                    val bookSort = spinnerPosToBookSort(binding.spSort.selectedItemPosition)
                    val coverPath = binding.ivCover.bitmapPath
                    val enableRefresh = false
                    val onlyUpdateRead = false
                    bookGroup?.let {
    ...
    companion object {
        private val spinnerToSort = mapOf(0 to 0, 1 to 2, 2 to 5, 3 to 6, 4 to 7, 5 to 8)
        private val sortToSpinner = mapOf(-1 to 0, 0 to 0, 1 to 0, 2 to 1, 3 to 0, 4 to 0, 5 to 2, 6 to 3, 7 to 4, 8 to 5)

        fun spinnerPosToBookSort(pos: Int) = spinnerToSort[pos] ?: 0
        fun bookSortToSpinnerPos(sort: Int) = sortToSpinner[sort] ?: 0
    }
